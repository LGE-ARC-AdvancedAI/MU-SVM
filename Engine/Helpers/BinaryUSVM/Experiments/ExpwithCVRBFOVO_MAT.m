function [RPred,RPredU,Remp,RempU,Finmodel,Finoutput,FinmodelU,FinoutputU,optparam]=ExpwithCVRBFOVO_MAT(trndata,tstdata,univdata,param)
%--------------------------------------------------------------------------
% DESCRIPTION: OVO model selection using separate validation data for RBF
% Kernel
%
% INPUT:
...
if(isempty(cset))
   % Wait for It!! Kaboooom!
    ...
    for k = 1:param.nfold
        ...
        %output.train = zeros(length(trndata.y),K,length(cset),length(gset));
        output.test = zeros(length(valdata.y),K,length(cset),length(gset));
        output.univ = zeros(length(univdata.y),K,length(cset),length(gset));
            ...
            for c = cset
                ...
                for g = gset
                    ...
                    for n = 1:length(valdata.y)
                        if(out.test.projection(n)>=0), output.test(n,CL(i,1),index1,index2) = output.test(n,CL(i,1),index1,index2)+1;
                        else output.test(n,CL(i,2),index1,index2) = output.test(n,CL(i,2),index1,index2)+1;     
                        end 
                    end
                end
            end
    ...
    for i=1:size(CL,1)
        ...
        %Finoutput.class(i).tstproj = out.test.projection'; Finoutput.class(i).tsty = tst.y;
...
%         FinoutputU.class(i).tstproj = out.test.projection'; FinoutputU.class(i).tsty = tst.y;
