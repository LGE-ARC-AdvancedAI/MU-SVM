function [RPred,RPredU,Remp,RempU,Finmodel,Finoutput,FinmodelU,FinoutputU,optparam]=ExpwithCVLinearOVA_MAT(trndata,tstdata,univdata,param)
%--------------------------------------------------------------------------
% DESCRIPTION: OVA model selection using separate validation data.
%
% INPUT:
    ...
    for k = 1:param.nfold
        ...
        for j=1:K
            ...
            for c = cset
                index1 = index1+1;
                param.c=c;
                [jnk,temp,model,out] =runUsvm(trn,val,[],param);
                output.test(valInd,j,index1) = out.test.projection';
            end
        end
...
% Use optimal model for Test Accuracy
for j=1:K
